...
class ImageRunner::ImageRunnerImpl {
  public:
    ImageRunnerImpl(const geometrize::Bitmap &targetBitmap)
        : m_model{targetBitmap} {}
    ImageRunnerImpl(const geometrize::Bitmap &targetBitmap,
                    const geometrize::Bitmap &initialBitmap)
        : m_model{targetBitmap, initialBitmap} {}
    ...
    step(const geometrize::ImageRunnerOptions &options,
        ...
        const auto [xMin, yMin, xMax, yMax] =
            geometrize::commonutil::mapShapeBoundsToImage(options.shapeBounds,
                                                          m_model.getTarget());
        const geometrize::ShapeTypes types = options.shapeTypes;

        if (!shapeCreator) {
            shapeCreator = geometrize::createDefaultShapeCreator(
                types, xMin, yMin, xMax, yMax);
        }

        m_model.setSeed(options.seed);
        return m_model.step(shapeCreator, options.alpha, options.shapeCount,
                            options.maxShapeMutations, options.maxThreads,
                            energyFunction, addShapePrecondition);
    }

    geometrize::Bitmap &getCurrent() { return m_model.getCurrent(); }

    geometrize::Bitmap &getTarget() { return m_model.getTarget(); }

    const geometrize::Bitmap &getCurrent() const {
        return m_model.getCurrent();
    }

    const geometrize::Bitmap &getTarget() const { return m_model.getTarget(); }

    geometrize::Model &getModel() { return m_model; }

  private:
    geometrize::Model m_model; ///< The model for the primitive
                               ///< optimization/fitting algorithm.
};

ImageRunner::ImageRunner(const geometrize::Bitmap &targetBitmap)
    : d{std::unique_ptr<ImageRunner::ImageRunnerImpl>(
...
geometrize::Bitmap &ImageRunner::getCurrent() { return d->getCurrent(); }

geometrize::Bitmap &ImageRunner::getTarget() { return d->getTarget(); }
...
geometrize::Model &ImageRunner::getModel() { return d->getModel(); }
