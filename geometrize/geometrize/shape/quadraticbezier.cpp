...
QuadraticBezier::QuadraticBezier(const float cx, const float cy, const float x1,
...
std::shared_ptr<geometrize::Shape> QuadraticBezier::clone() const {
    std::shared_ptr<geometrize::QuadraticBezier> bezier{
        std::make_shared<geometrize::QuadraticBezier>()};
    bezier->m_x1 = m_x1;
    bezier->m_y1 = m_y1;
    bezier->m_cx = m_cx;
    bezier->m_cy = m_cy;
