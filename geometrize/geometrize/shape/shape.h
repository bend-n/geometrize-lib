...
class Shape {
  public:
    ...
    std::function<std::vector<geometrize::Scanline>(const geometrize::Shape &)>
        rasterize;
    ...
    virtual std::shared_ptr<geometrize::Shape> clone() const {
        throw "Unimplemented"; // NOTE not pure virtual because it breaks simple
                               // upcast in Chaiscript (and can't see how else
                               // to do it)
    }
     ...
     */
    virtual geometrize::ShapeTypes getType() const { throw "Unimplemented"; }
};
