...
/**
 * @brief hillClimb Hill climbing optimization algorithm, attempts to minimize
 * energy (the error/difference).
 ...
 */
geometrize::State
hillClimb(const geometrize::State &state, const std::uint32_t maxAge,
          const geometrize::Bitmap &target, const geometrize::Bitmap &current,
          geometrize::Bitmap &buffer, const double lastScore,
    ...
    while (age < maxAge) {
        ...
        s.m_score = energyFunction(s.m_shape->rasterize(*s.m_shape), s.m_alpha,
                                   target, current, buffer, lastScore);
 ...
 * @param shapeCreator A function that will create the shapes that will be
 * chosen from.
...
geometrize::State bestRandomState(
    ...
    const std::uint32_t alpha, const std::uint32_t n,
    const geometrize::Bitmap &target, const geometrize::Bitmap &current,
    geometrize::Bitmap &buffer, const double lastScore,
    const geometrize::core::EnergyFunction &energyFunction) {
    geometrize::State bestState(shapeCreator(), alpha);
    bestState.m_score =
        energyFunction(bestState.m_shape->rasterize(*bestState.m_shape),
    ...
    for (std::uint32_t i = 0; i <= n; i++) {
        ...
        state.m_score =
            energyFunction(state.m_shape->rasterize(*state.m_shape),
...
double defaultEnergyFunction(const std::vector<geometrize::Scanline> &lines,
                             const std::uint32_t alpha,
                             const geometrize::Bitmap &target,
                             const geometrize::Bitmap &current,
                             geometrize::Bitmap &buffer, const double score) {
    const geometrize::rgba color(geometrize::core::computeColor(
        target, current, lines,
        alpha)); // Calculate best color for areas covered by the scanlines
    geometrize::copyLines(
        buffer, current,
        lines); // Copy area covered by scanlines to buffer bitmap
    geometrize::drawLines(buffer, color,
                          lines); // Blend scanlines into the buffer using the
                                  // color calculated earlier
    return geometrize::core::differencePartial(
        target, current, buffer, score,
        lines); // Get error measure between areas of current and modified
                // buffers covered by scanlines
}

geometrize::rgba computeColor(const geometrize::Bitmap &target,
                              const geometrize::Bitmap &current,
    ...
    std::int64_t count{0};
    const std::int32_t a{
        static_cast<std::int32_t>(257.0f * 255.0f / static_cast<float>(alpha))};
    ...
    for (const geometrize::Scanline &line : lines) {
        ...
        for (std::int32_t x = line.x1; x <= line.x2; x++) {
            ...
            // Mix the red, green and blue components, blending by the given
            // alpha value
    ...
    const std::uint8_t r{static_cast<std::uint8_t>(
        commonutil::clamp(rr, INT32_C(0), INT32_C(255)))};
    const std::uint8_t g{static_cast<std::uint8_t>(
        commonutil::clamp(gg, INT32_C(0), INT32_C(255)))};
    const std::uint8_t b{static_cast<std::uint8_t>(
        commonutil::clamp(bb, INT32_C(0), INT32_C(255)))};

    return geometrize::rgba{r, g, b, alpha};
}

double differenceFull(const geometrize::Bitmap &first,
    ...
    for (std::size_t y = 0; y < height; y++) {
        for (std::size_t x = 0; x < width; x++) {
            ...
            const std::int32_t dr = {static_cast<std::int32_t>(f.r) -
                                     static_cast<std::int32_t>(s.r)};
            const std::int32_t dg = {static_cast<std::int32_t>(f.g) -
                                     static_cast<std::int32_t>(s.g)};
            const std::int32_t db = {static_cast<std::int32_t>(f.b) -
                                     static_cast<std::int32_t>(s.b)};
            const std::int32_t da = {static_cast<std::int32_t>(f.a) -
                                     static_cast<std::int32_t>(s.a)};
...
double differencePartial(const geometrize::Bitmap &target,
                         const geometrize::Bitmap &before,
                         const geometrize::Bitmap &after, const double score,
    ...
    std::uint64_t total{static_cast<std::uint64_t>(
        (score * 255.0) * (score * 255.0) * rgbaCount)};
    for (const geometrize::Scanline &line : lines) {
        const std::int32_t y{line.y};
        for (std::int32_t x = line.x1; x <= line.x2; x++) {
            const geometrize::rgba t(target.getPixel(x, y));
            ...
            const std::int32_t dtbr{static_cast<std::int32_t>(t.r) -
                                    static_cast<std::int32_t>(b.r)};
            const std::int32_t dtbg{static_cast<std::int32_t>(t.g) -
                                    static_cast<std::int32_t>(b.g)};
            const std::int32_t dtbb{static_cast<std::int32_t>(t.b) -
                                    static_cast<std::int32_t>(b.b)};
            const std::int32_t dtba{static_cast<std::int32_t>(t.a) -
                                    static_cast<std::int32_t>(b.a)};

            const std::int32_t dtar{static_cast<std::int32_t>(t.r) -
                                    static_cast<std::int32_t>(a.r)};
            const std::int32_t dtag{static_cast<std::int32_t>(t.g) -
                                    static_cast<std::int32_t>(a.g)};
            const std::int32_t dtab{static_cast<std::int32_t>(t.b) -
                                    static_cast<std::int32_t>(a.b)};
            const std::int32_t dtaa{static_cast<std::int32_t>(t.a) -
                                    static_cast<std::int32_t>(a.a)};

            total -= static_cast<std::uint64_t>(dtbr * dtbr + dtbg * dtbg +
                                                dtbb * dtbb + dtba * dtba);
            total += static_cast<std::uint64_t>(dtar * dtar + dtag * dtag +
                                                dtab * dtab + dtaa * dtaa);
        }
    }

    const double result{
        std::sqrt(static_cast<double>(total) / static_cast<double>(rgbaCount)) /
        255.0};
    return result;
}
...
geometrize::State bestHillClimbState(
    const std::function<std::shared_ptr<geometrize::Shape>(void)> &shapeCreator,
    const std::uint32_t alpha, const std::uint32_t n, const std::uint32_t age,
    const geometrize::Bitmap &target, const geometrize::Bitmap &current,
    geometrize::Bitmap &buffer, const double lastScore,
    const EnergyFunction &customEnergyFunction) {
    const EnergyFunction &e = customEnergyFunction
                                  ? customEnergyFunction
                                  : geometrize::core::defaultEnergyFunction;

    const geometrize::State state{bestRandomState(
        shapeCreator, alpha, n, target, current, buffer, lastScore, e)};
