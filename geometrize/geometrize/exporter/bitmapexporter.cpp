...
inline void writeToStream(std::ostringstream &stream, const std::uint32_t &t) {
    if (bigEndian()) {
        ...
        stream.write(reinterpret_cast<const char *>(&flipped),
                     sizeof(std::uint32_t));
    } else {
...
std::string exportBMP(const geometrize::Bitmap &bitmapData) {
    ...
    const std::uint32_t width{
        bitmapData.getWidth()}; // The width of the image in pixels.
    const std::uint32_t height{
        bitmapData.getHeight()}; // The height of the image in pixels.

    // Per row pad byte count, used to ensure that each row is a multiple of 4
    // bytes.
    const std::uint32_t padding{((width * 3) % 4 != 0) ? 4 - ((width * 3) % 4)
                                                       : 0};

    // Bitmap Information Header
    const std::uint32_t ifSize{
        BITMAP_INFORMATION_HEADER_SIZE}; // The number of bytes required by the
                                         // structure.
    const std::uint16_t planes{1U};      // The number of planes for the target
                                         // device. This value must be set to 1.
    const std::uint16_t bitCount{
        24U}; // The number of bits that define each pixel and the maximum
              // number of colors in the bitmap.
    const std::uint32_t compression{
        0U}; // Specifies the bitmap compression type.
    const std::uint32_t imageSize{(((width * 3U) + padding) & 0x0000FFFC) *
                                  height}; // The size of the image in bytes.
    const std::uint32_t xPelsPerMeter{
        0U}; // The horizontal resolution in pixels-per-meter.
    const std::uint32_t yPelsPerMeter{
        0U}; // The vertical resolution in pixels-per-meter.
    const std::uint32_t colorsUsed{
        0U}; // The number of color indexes in the color table actually used by
             // the bitmap.
    const std::uint32_t colorsImportant{
        0U}; // The number of color indexes required for displaying the bitmap.
             // If zero, all colors are required.

    // Bitmap File Header
    const std::uint16_t type{19778U}; // The file type.
    const std::uint32_t fhSize{
        BITMAP_INFORMATION_HEADER_SIZE + BITMAP_FILE_HEADER_SIZE +
        imageSize};                    // The size in bytes of the bitmap file.
    ...
    const std::uint32_t offbits{
        BITMAP_INFORMATION_HEADER_SIZE +
        BITMAP_FILE_HEADER_SIZE}; // The offset, in bytes, from the beginning of
