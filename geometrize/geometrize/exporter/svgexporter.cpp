...
std::string getSvgShapeData(const geometrize::Circle &s) {
    ...
    strm << "<circle cx=\"" << s.m_x << "\" cy=\"" << s.m_y << "\" r=\""
         << s.m_r << "\" " << geometrize::exporter::SVG_STYLE_HOOK << " />";
    return strm.str();
}

std::string getSvgShapeData(const geometrize::Ellipse &s) {
    std::stringstream strm;
    strm << "<ellipse cx=\"" << s.m_x << "\" cy=\"" << s.m_y << "\" rx=\""
         << s.m_rx << "\" ry=\"" << s.m_ry << "\" "
         << geometrize::exporter::SVG_STYLE_HOOK << " />";
    return strm.str();
}

std::string getSvgShapeData(const geometrize::Line &s) {
    std::stringstream strm;
    strm << "<line x1=\"" << s.m_x1 << "\" y1=\"" << s.m_y1 << "\" x2=\""
         << s.m_x2 << "\" y2=\"" << s.m_y2 << "\" "
         << geometrize::exporter::SVG_STYLE_HOOK << " />";
    return strm.str();
}
...
std::string getSvgShapeData(const geometrize::QuadraticBezier &s) {
    std::stringstream strm;
    strm << "<path d=\"M" << s.m_x1 << " " << s.m_y1 << " Q " << s.m_cx << " "
         << s.m_cy << " " << s.m_x2 << " " << s.m_y2 << "\" "
         << geometrize::exporter::SVG_STYLE_HOOK << " />";
    return strm.str();
}
...
std::string getSvgShapeData(const geometrize::Rectangle &s) {
    std::stringstream strm;
    strm << "<rect x=\"" << (std::fmin)(s.m_x1, s.m_x2) << "\" y=\""
         << (std::fmin)(s.m_y1, s.m_y2) << "\" width=\""
         << (std::fmax)(s.m_x1, s.m_x2) - (std::fmin)(s.m_x1, s.m_x2)
         << "\" height=\""
         << (std::fmax)(s.m_y1, s.m_y2) - (std::fmin)(s.m_y1, s.m_y2) << "\" "
         << geometrize::exporter::SVG_STYLE_HOOK << " />";
    return strm.str();
}

std::string
getSvgShapeData(const geometrize::RotatedEllipse &s,
                const geometrize::exporter::RotatedEllipseSVGExportMode mode) {
    std::stringstream strm;

    switch (mode) {
    case geometrize::exporter::RotatedEllipseSVGExportMode::ELLIPSE_ITEM: {
        strm << "<g transform=\"translate(" << s.m_x << " " << s.m_y
             << ") rotate(" << s.m_angle << ") scale(" << s.m_rx << " "
             << s.m_ry << ")\">"
             << "<ellipse cx=\"" << 0 << "\" cy=\"" << 0 << "\" rx=\"" << 1
             << "\" ry=\"" << 1 << "\" " << geometrize::exporter::SVG_STYLE_HOOK
             << " />"
             << "</g>";
    } break;
    case geometrize::exporter::RotatedEllipseSVGExportMode::POLYGON: {
        ...
        std::vector<std::pair<float, float>> points{
            geometrize::getPointsOnRotatedEllipse(s, pointCount)};
...
std::string getSvgShapeData(const geometrize::Triangle &s) {
    std::stringstream strm;
    strm << "<polygon points=\"" << s.m_x1 << "," << s.m_y1 << " " << s.m_x2
         << "," << s.m_y2 << " " << s.m_x3 << "," << s.m_y3 << "\" "
         << geometrize::exporter::SVG_STYLE_HOOK << " "
         << "/>";
    return strm.str();
}
...
std::string
getSvgShapeData(const geometrize::Shape &s,
    ...
    case geometrize::ShapeTypes::ROTATED_RECTANGLE:
        return getSvgShapeData(
            static_cast<const geometrize::RotatedRectangle &>(s));
    ...
    case geometrize::ShapeTypes::ROTATED_ELLIPSE:
        return getSvgShapeData(
            static_cast<const geometrize::RotatedEllipse &>(s),
            options.rotatedEllipseExportMode);
    ...
    case geometrize::ShapeTypes::QUADRATIC_BEZIER:
        return getSvgShapeData(
            static_cast<const geometrize::QuadraticBezier &>(s));
...
std::string getSVGRgbColorAttrib(const geometrize::rgba color) {
    ...
    stream << "rgb(" << static_cast<std::int32_t>(color.r) << ","
...
std::string
getSingleShapeSVGData(const geometrize::rgba &color,
    ...
    shapeData = std::regex_replace(
        shapeData, std::regex(geometrize::exporter::SVG_STYLE_HOOK), styles);
...
std::string getSingleShapeSVGData(const geometrize::rgba &color,
                                  const geometrize::Shape &shape,
                                  SVGExportOptions options) {
    return ::getSingleShapeSVGData(color, shape, options);
}

std::string exportSingleShapeSVG(const geometrize::rgba &color,
                                 const geometrize::Shape &shape,
    ...
    stream << "<?xml version=\"1.0\" standalone=\"no\"?>"
           << "\n";
    stream << "<svg xmlns=\"https://www.w3.org/2000/svg\" version=\"1.2\" "
              "baseProfile=\"tiny\" "
           << "width=\"" << width << "\" "
           << "height=\"" << height << "\" "
