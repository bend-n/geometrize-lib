...
/**
 * @brief The Scanline class represents a scanline, a row of pixels running
 * across a bitmap.
...
bool operator==(const geometrize::Scanline &lhs,
                const geometrize::Scanline &rhs);
bool operator!=(const geometrize::Scanline &lhs,
                const geometrize::Scanline &rhs);

/**
 * @brief trimScanlines Crops the scanning width of an array of scanlines so
 * they do not scan outside of the given area.
...
trimScanlines(const std::vector<geometrize::Scanline> &scanlines,
              std::int32_t minX, std::int32_t minY, std::int32_t maxX,
              std::int32_t maxY);
